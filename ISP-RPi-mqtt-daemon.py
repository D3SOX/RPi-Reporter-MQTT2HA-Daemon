...
def getFileSystemDrives():
    # Get list of drives using df command
    stdout, _, returncode = invoke_shell_cmd('df -h')
    if returncode != 0:
        print_line('Failed to get drive information', error=True)
        return []

    # Split into lines and clean up
    lines = stdout.decode('utf-8').split('\n')
    trimmedLines = [line.strip() for line in lines if line.strip()]
    ...
    for currLine in trimmedLines:
        ...
        # Find the percentage field index
        percent_field_index = -1
        for i, part in enumerate(lineParts):
            if '%' in part:
                percent_field_index = i
                break
                
        if percent_field_index == -1:
            print_line('No percentage field found, Skipped=[{}]'.format(lineParts), debug=True, warning=True)
            continue
        ...
        try:
            # Convert size to number, handling different units
            size_str = lineParts[total_size_idx].upper()
            size_multiplier = 1
            if 'G' in size_str:
            ...
            size = float(size_str) * size_multiplier
            used_percent = int(lineParts[percent_field_index].replace('%', ''))
            mount_point = lineParts[mount_idx]

            tmpDrives.append((size, used_percent, mount_point, device))
            print_line('Added drive: size={}, used={}%, mount={}, device={}'.format(
                size, used_percent, mount_point, device), debug=True)
                
        except (ValueError, IndexError) as e:
            print_line('Error parsing drive info: {}'.format(e), error=True)
            continue
...
def send_status(timestamp, nothing):
    ...
    # Handle filesystem space with error checking
    try:
        if rpi_filesystem_space and rpi_filesystem_space.strip():
            space_str = rpi_filesystem_space.replace('GB', '').strip()
            if space_str:
                rpiData[K_RPI_FS_SPACE] = int(space_str, 10)
            else:
                rpiData[K_RPI_FS_SPACE] = 0
        else:
            rpiData[K_RPI_FS_SPACE] = 0
    except (ValueError, AttributeError) as e:
        print_line(f'Error parsing filesystem space: {e}', error=True)
        rpiData[K_RPI_FS_SPACE] = 0

    # Handle filesystem percentage with error checking
    try:
        if rpi_filesystem_percent and rpi_filesystem_percent.strip():
            percent_str = rpi_filesystem_percent.replace('%', '').strip()
            if percent_str:
                used_percent = int(percent_str, 10)
                rpiData[K_RPI_FS_AVAIL] = 100 - used_percent
                rpiData[K_RPI_FS_USED] = used_percent
            else:
                rpiData[K_RPI_FS_AVAIL] = 0
                rpiData[K_RPI_FS_USED] = 0
        else:
            rpiData[K_RPI_FS_AVAIL] = 0
            rpiData[K_RPI_FS_USED] = 0
    except (ValueError, AttributeError) as e:
        print_line(f'Error parsing filesystem percentage: {e}', error=True)
        rpiData[K_RPI_FS_AVAIL] = 0
        rpiData[K_RPI_FS_USED] = 0
